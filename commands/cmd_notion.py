...
def create_database(ctx,parent, db, props,):
    ...
    props = props.split(",")
    res =  notion_svc.create_database(parent=parent,database_name=db,list_props=props)
...
def update_db(ctx,db,new_name, props):
    ...
    res = notion_svc.update_database(database_name=db,new_name= new_name, new_props=props)
